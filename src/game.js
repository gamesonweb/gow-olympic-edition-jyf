import { ArcRotateCamera, BoundingInfo, Color3, Color4, CreateGreasedLine, CubeTexture, DefaultRenderingPipeline, DirectionalLight, FreeCamera, GreasedLineMeshColorDistribution, GreasedLineMeshColorMode, GreasedLineRibbonPointsMode, GreasedLineTools, HemisphericLight, KeyboardEventTypes, MeshBuilder, MotionBlurPostProcess, ParticleSystem, Scalar, Scene, SceneLoader, ShadowGenerator, Sound, StandardMaterial, Texture, Tools, TransformNode, Vector3 } from "@babylonjs/core";
...
import trempUrl from "/Users/louafiyasser/projetweb24/assets/models/tremp.glb";
...
class Game {
    ...
    moveRamp() {
        // Déplace le tremplin vers l'avant
        this.scene.registerBeforeRender(() => {
            this.tremp.position.z -= SPEED_Z * this.delta;
            
            // Si le tremplin sort de la vue, réinitialisez sa position
            if (this.tremp.position.z < -100) {
                this.tremp.position.z = SPAWN_POS_Z; // Réinitialiser la position du tremplin
            }
        });
    }
    ...
    async createScene() {
        ...
        //let obstacleModele = MeshBuilder.CreateBox("obstacle", { width: 0.5, height: 1, depth: 1 }, this.scene);
         res = await SceneLoader.ImportMeshAsync("", "", trempUrl, this.scene);
         //let res = await SceneLoader.ImportMeshAsync("", "", trempUrl, this.scene);
        this.tremp = res.meshes[0];
        this.tremp.scaling = new Vector3(10, 10, 10); // Échelle du tremplin si nécessaire
        
        this.tremp.position.set(Scalar.RandomRange( 0, 0, SPAWN_POS_Z ) ); // Position initiale du tremplin
        // ...
         // Appeler la fonction pour déplacer le tremplin
       this.moveRamp();  
